...
Dies führt dazu, dass der Gradient im Frequenzraum nicht ausreichens genau bestimmt werden kann. Eine Alternative dazu ist die Methode über die Halbwertsbreite.\\
Die grafisch ermittelte Halbwertsbreite $t_\frac{1}{2}$ ist dabei in Abbildung \ref{img:four1} abgebildet.
Sie ergibt sich zu 
\begin{equation*}
  t_\frac{1}{2} = \SI{8 e-5}{\milli\second} \quad.
\end{equation*}
...
\begin{align*}
  ...
  \Leftrightarrow G_\t{FWHM} &= \frac{4 \cdot 2.2}{d \gamma t_\frac{1}{2}} = \SI{0.0981}{\tesla\per\metre} \quad.
\end{align*}
...
\begin{equation*}
  G_\t{Fourier} = \frac{2\pi d_\text{f}}{\gamma d} = \SI{0.0984}{\tesla\per\metre} \quad.
\end{equation*}
Dabei ist  $d = \SI{4.2}{\milli\metre}$ \cite{V49} der Durchmesser des Probenröhrchens, $d_\text{f} \approx \SI{17564}{\hertz}$ der Durchmesser im Frequenzraum und
...
\begin{align*}
  D_\t{Fourier}&  = \frac{3}{2\cdot T_\t{D} \gamma^2 G^2} &= \SI{1.29(001)e-9}{\metre^2\per\second}\\
  D_\t{FWHM}&  &= \SI{ 1.30(1)e-9}{\metre^2\per\second}
\end{align*}
berechnen. 
